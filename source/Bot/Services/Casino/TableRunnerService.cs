...
namespace Bot.Services.Casino
{
        ...
        where TTableDetails : CasinoTableDetails<TTable, TPlayer, THand>
    {
        ...
        /// </summary>
        protected ILogger<TableRunnerService<THand, TPlayer, TTable, TTableDetails>> Logger => _logger;
        ...
        /// </remarks>
        protected ConcurrentDictionary<ulong, TTableDetails> Tables => _tables;


        protected TableRunnerService(
        ...
        /// <returns>Boolean value to indicate whether the thread has been started (or is already started)</returns>
        ...
        {
            if (table.PendingPlayers.All(pp => pp.User.UserId != user.Id) && table.Players.All(p => p.User.UserId != user.Id))
            {
                var userData = await _userService.GetOrCreateUserData(user);
                var casinoPlayer = CreatePlayer(userData, user);
                if (table.IsGameActive)
                {
                    Logger.LogInformation("Adding {player} to the Pending Players collection", casinoPlayer.Name);
                    table.PendingPlayers.Add(casinoPlayer);
                }
                else
                {
                    Logger.LogInformation("Adding {player} to the Players collection", casinoPlayer.Name);
                    table.Players.Add(casinoPlayer);
                }
                return true;
            }
        ...
        {
            switch (table.IsGameActive)
            {
                case true when table.LeavingPlayers.All(lp => lp.User.UserId != user.Id):
                    ...
                    return true;
                case false when table.Players.Any(p => p.User.UserId == user.Id):
        ...
        protected abstract TPlayer CreatePlayer(EileenUserData userData, IUser user);
        ...
        /// <returns><typeparamref name="TTable"/></returns>
        protected abstract TTable CreateNewTable(IThreadChannel threadChannel);
        ...
        protected abstract TTableDetails CreateTableDetails(TTable table, IThreadChannel channel);
        ...
        protected abstract Task TableRunnerLoop(TTableDetails tableDetails);
